#ifndef POTS_H
#define POTS_H

#include "constants.h"
#include <ResponsiveAnalogRead.h>

const int POT_L_PINS[] = {POT_L_1, POT_L_2, POT_L_3};
const int POT_R_PINS[] = {POT_R_1, POT_R_2, POT_R_3};

int midiLCState[3] = {0}, midiLPState[3] = {0};
int midiRCState[3] = {0}, midiRPState[3] = {0};

const float snapMultiplier = 0.05;
const int potMin = 10;
const int potMax = 1023;

ResponsiveAnalogRead responsivePotL[3] = {
    ResponsiveAnalogRead(POT_L_1, true, snapMultiplier),
    ResponsiveAnalogRead(POT_L_2, true, snapMultiplier),
    ResponsiveAnalogRead(POT_L_3, true, snapMultiplier)};
ResponsiveAnalogRead responsivePotR[3] = {
    ResponsiveAnalogRead(POT_R_1, true, snapMultiplier),
    ResponsiveAnalogRead(POT_R_2, true, snapMultiplier),
    ResponsiveAnalogRead(POT_R_3, true, snapMultiplier)};

void setupPots()
{
    for (int i = 0; i < 3; i++)
    {
        responsivePotL[i].setAnalogResolution(1023);
        responsivePotR[i].setAnalogResolution(1023);

        // Lê e envia a posição física real no boot
        // Garante que o Serato abre com o valor do pot, não com zero
        int valL = constrain(map(analogRead(POT_L_PINS[i]), potMin, potMax, 0, 127), 0, 127);
        usbMIDI.sendControlChange(POT_L_CC_START + i, valL, MIDI_CH);
        midiLPState[i] = valL;

        int valR = constrain(map(analogRead(POT_R_PINS[i]), potMin, potMax, 0, 127), 0, 127);
        usbMIDI.sendControlChange(POT_R_CC_START + i, valR, MIDI_CH);
        midiRPState[i] = valR;
    }
}

void readPots()
{
    for (int i = 0; i < 3; i++)
    {
        // Lado L
        responsivePotL[i].update(analogRead(POT_L_PINS[i]));
        if (responsivePotL[i].hasChanged())
        {
            midiLCState[i] = constrain(map(responsivePotL[i].getValue(), potMin, potMax, 0, 127), 0, 127);
            if (midiLCState[i] != midiLPState[i])
            {
                usbMIDI.sendControlChange(POT_L_CC_START + i, midiLCState[i], MIDI_CH);
                if (DEBUG)
                {
                    Serial.print("POT_L_");
                    Serial.print(i + 1);
                    Serial.print(": ");
                    Serial.println(midiLCState[i]);
                }
                midiLPState[i] = midiLCState[i];
            }
        }

        // Lado R
        responsivePotR[i].update(analogRead(POT_R_PINS[i]));
        if (responsivePotR[i].hasChanged())
        {
            midiRCState[i] = constrain(map(responsivePotR[i].getValue(), potMin, potMax, 0, 127), 0, 127);
            if (midiRCState[i] != midiRPState[i])
            {
                usbMIDI.sendControlChange(POT_R_CC_START + i, midiRCState[i], MIDI_CH);
                if (DEBUG)
                {
                    Serial.print("POT_R_");
                    Serial.print(i + 1);
                    Serial.print(": ");
                    Serial.println(midiRCState[i]);
                }
                midiRPState[i] = midiRCState[i];
            }
        }
    }
}

#endif